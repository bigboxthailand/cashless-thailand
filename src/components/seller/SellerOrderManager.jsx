...
const SellerOrderManager = ({ shopId: propShopId }) => {
    ...
    const [shipData, setShipData] = useState({ orderId: '', currentTracking: '', newTracking: '' });
    ...
    const openShipModal = (orderId, tracking) => {
        setShipData({ orderId, currentTracking: tracking || '', newTracking: '' });
    ...
    const handleShipUpdate = async (e) => {
        ...
        setIsSaving(true);
        try {
            const { orderId, currentTracking, newTracking } = shipData;
            ...
            const { error } = await supabase
                .from('orders')
                .update({
                    shipping_status: 'shipped', // MVP: Mark global as shipped
                    ...
                    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl w-full max-w-md p-8 relative z-10 shadow-2xl animate-fade-in-up">
                        ...
                        <form onSubmit={handleShipUpdate} className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Tracking Number</label>
                                <input
                                    type="text"
                                    required
                                    placeholder="e.g. KERRY-123456"
                                    value={shipData.newTracking}
                                    onChange={e => setShipData({ ...shipData, newTracking: e.target.value })}
                                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#D4AF37] outline-none font-mono"
                                />
                                <p className="text-[10px] text-white/30">If other shops already added tracking, this will append to it.</p>
                            </div>
